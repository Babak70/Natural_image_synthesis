...
Code adopted from McIntosh et. al. (2016),
"""
...
def correlation_coefficient(obs_rate, est_rate):
    ...
    x_std = K.std(obs_rate, axis=0, keepdims=True)+np.finfo(float).eps
  

    y_mu = est_rate - K.mean(est_rate, axis=0, keepdims=True)

    y_std = K.std(est_rate, axis=0, keepdims=True)+np.finfo(float).eps
...
#     x_mu = obs_rate - K.mean(obs_rate)


#     x_std = K.std(obs_rate)
  

#     y_mu = est_rate - K.mean(est_rate)

#     y_std = K.std(est_rate)

#     return K.mean(x_mu * y_mu) / (x_std * y_std)
...
def fraction_of_explained_variance(obs_rate, est_rate):
    ...
    return 1.0 - mean_squared_error(obs_rate, est_rate) /( K.var(obs_rate, axis=0, keepdims=True)+np.finfo(float).eps)
