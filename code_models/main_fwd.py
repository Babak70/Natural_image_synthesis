...
from train_test_fwd import train_forward
from dim_reduction_utils import tSNE_transform
from Fwd_Models import ML_fwd
import logging
        ...
        for phase in ['train', 'val', 'test']:    
          ...
          if phase =='train':
            ...
            for n, (stimuli, spikes) in test_ds.enumerate():
               stimuli=tf.transpose(stimuli,perm=[1,0,2])
               # spikes=tf.squeeze(spikes,axis=[0])
               est_rate_t,obs_rate_t,loss_t,corr_t= test_forward(stimuli, spikes, epoch + num_of_epochs*(iter_i),'generation',model_sub_local)
               assert tf.math.is_finite(loss_t)
               running_loss += loss_t.numpy()
               running_corr +=corr_t.numpy()
               running_size += 1
...
def actor_model_iter(n_iter,model,model_sub_optimizer):
...
train_fwd, val_fwd,  test_fwd = actor_model_iter(n_iter,model,model_sub_optimizer)
